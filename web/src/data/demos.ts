...
export const demos: Record<string, Demo> = {
  ...
  blame: {
    ...
    caption: 'Mixed-agent blame shows the prompt behind each line, not just a hash.',
    out: [
      `14:57 ${gr('[claude 15:02]')} turn 1  ${dim('21')} | export async function receiveStripe(req, res) {`,
      `  ${dim('Prompt: "Trace why webhook retries return 500 after the order was already fulfilled..."')}`,
      `15:06 ${gr('[claude 15:02]')} turn 2  ${dim('22')} |   const event = verify(req.rawBody, signature)`,
      `  ${dim('Prompt: "Signature checks fail in production because Express parsed the Stripe body first..."')}`,
      `15:12 ${gr('[codex 15:08]')} turn 2   ${dim('23')} |   const claimed = await claims.insert(event.id)`,
      `  ${dim('Prompt: "The Stripe webhook can be delivered twice. Claim the event id in Postgres..."')}`,
