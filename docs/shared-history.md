...
Workspace paths normalize to `$WORKSPACE` only when their boundary is unambiguous: the root ends the field, continues through a path separator without a `..` component, or is enclosed by matching quotes. Because Unix permits whitespace and punctuation inside filenames, an unquoted workspace-root mention followed by either is ambiguous and causes the entire field to become `[PATH_REDACTED]`. Any other absolute path redacts all non-normalized content in the field; an explicitly quoted `$WORKSPACE` path may remain only when its suffix has no ambiguous delimiter followed by another separator. This fail-closed rule trades some shared prose for a stable privacy boundary; quote a standalone path when precise normalization matters.
