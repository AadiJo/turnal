...
Before approving the version 3 scanner, upgrade every device that pulls the shared history. Older readers reject its detector-specific manifest metadata: `sync pull` exits nonzero and quarantines the publisher without advancing its observation cursor. Upgrade the affected reader and run `turnal sync pull` again. A successful pull clears the quarantine automatically. Updated readers continue to accept older manifests. Inline `gitleaks:allow` and `betterleaks:allow` comments cannot disable publication redaction.
