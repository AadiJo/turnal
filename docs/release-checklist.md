...
- Before enabling the first publisher, trigger a CircleCI pipeline on `main` with `rehearse_release` set to `true`. It repeats every gate, acquires the npm-audience OIDC token, builds the package and SBOM, and exits before npm or GitHub publication.
