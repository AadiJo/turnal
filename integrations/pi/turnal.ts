...
function capture(hook: string, payload: Payload): Promise<void> {
  return new Promise((resolve) => {
    ...
    const child = spawn("turnal", ["adapter", "capture", "pi", hook], {
...
export default function turnal(pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    lastAssistantText = "";
    await capture("before_agent_start", { ...sessionPayload(ctx, parentSessionID), prompt: event.prompt });
  });
