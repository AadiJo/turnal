...
> **Secret scanning is best-effort.** Always read `turnal share preview --json` before approving a policy. Allowed text can still contain source fragments, and the scanner detects common credential shapes rather than every possible secret. Prefer `omit` or `metadata_only` for a first pilot.
