...
func (manager Manager) ClearActiveForRecovery(sessionID primitives.SessionID, expectedTurnID primitives.TurnID) error {
	...
	return manager.Repo.WithWorkspaceLock("clear recovered active turn", func() error {
		return manager.ClearActiveForRecoveryLocked(sessionID, expectedTurnID)
	})
}

// ClearActiveForRecoveryLocked clears recovered state while the caller holds
// the workspace lock.
func (manager Manager) ClearActiveForRecoveryLocked(sessionID primitives.SessionID, expectedTurnID primitives.TurnID) error {
	if err := manager.validate(); err != nil {
		return err
	}
	active, err := manager.readActive(sessionID)
