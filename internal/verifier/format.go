...
func WriteHuman(writer io.Writer, report Report) error {
	...
	if _, err := fmt.Fprintf(writer, "checks: %d passed, %d failed, %d timed out, %d could not start\n\n",
		report.Summary.Passed, report.Summary.Failed, report.Summary.TimedOut, report.Summary.LaunchError); err != nil {
		return err
	}
