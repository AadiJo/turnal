...
func normalizeAgent(value string) (string, error) {
	...
	switch value {
	case "auto", "claude", "codex", "copilot", "cursor", "gemini", "opencode", "pi", "all", "none":
		return value, nil
	default:
		return "", fmt.Errorf("invalid init.agent %q; expected auto, claude, codex, copilot, cursor, gemini, opencode, pi, all, or none", value)
	}
}
